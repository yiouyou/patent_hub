	...
	try:
		...
		with atomic_transaction():
			locked = frappe.db.sql(
				"""
                SELECT is_done_md2docx AS done, is_running_md2docx AS running
                FROM `tabMd2docx`
		...
		return enqueue_long_task(
			...
			task_key=TASK_KEY,
			prefix=STEP_PREFIX,
			job_method="patent_hub.api.call_md2docx._job",
			queue="long",
			timeout=TIMEOUT,
...
def _validate_required_fields(doc) -> str | None:
	"""验证必填字段"""
	if not isinstance(doc.md, str) or not doc.md.strip():
		return "md 字段不能为空"
	return None
...
def _job(doctype: str, docname: str, task_key: str, *, force: bool = False):
	...
	- 成功：complete_task_fields（自动 realtime: md2docx_done）
	- 失败：fail_task_fields（自动 realtime: md2docx_failed）
	"""
	...
	try:
		...
		# 读取必要输入（避免长事务）
		md, is_patent = frappe.db.get_value(DOCTYPE, docname, ["md", "is_patent_md2docx"])
		if not isinstance(md, str) or not md.strip():
			raise ValueError("缺少 md 内容")
...
def _process_api_result(docname: str, result: dict, user: str | None = None):
	with atomic_transaction():
		...
		# 保存 DOCX
		docx_bytes = res_data.get("docx_bytes")
		...
		if isinstance(docx_bytes, (bytes, bytearray)):
			file_doc = _save_docx_file(doc, bytes(docx_bytes))
			doc.set("md_docx_link", file_doc.name)

		# 统一完成（会自动 publish_realtime: md2docx_done）
		complete_task_fields(
			doc,
			TASK_KEY,
			extra_fields={
...
def _save_docx_file(doc, docx_bytes: bytes):
	...
	filename = f"{getattr(doc, f'{TASK_KEY}_id')}_md.docx"
	try:
